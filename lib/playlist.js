...
// and compose the settings object the kiosk consumes (surface from the active
// Station, diaporama playback from the active Scene).
...
function activeInfo(machine) {
    ...
    if (!a) return null;
    const show = defaults.cleanMediaFilter(a.station && a.station.mediaFilter);
    return {
        projectId: a.projectId, sceneId: a.sceneId, sceneName: a.sceneName,
        stream: !!(a.source.accept && a.source.accept.stream) && show.stream,
        streamMode: a.source.streamMode === 'grid' ? 'grid' : 'replace',
        // NDI: the scene toggles it on; the SOURCE name comes from the station (the box in this project)
        ndi: {
            on: !!(a.source.ndi && a.source.ndi.on) && show.ndi,
            source: (a.station && a.station.ndi && a.station.ndi.source) || ''
        }
    };
}
...
function playlist(uploadPath, machine) {
    ...
    const show = defaults.cleanMediaFilter(a.station && a.station.mediaFilter);
    return orderMedia(listMedia(dir), a.source.order)
        .filter(m => show[m.type])
        .map(m => ({
...
// the settings object for the kiosk (surface from the active station, playback
// from the active scene)
function settingsFor(machine) {
    const a = activeOf(machine);
    return defaults.composeSettings(machine, a ? a.station : null, a ? a.source : null);
}
