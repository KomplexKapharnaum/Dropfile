...
// resolve a project by its public "drop home" token (the menu page)
function findProjectByHomeToken(homeToken) {
    if (!homeToken) return null;
    return Object.values(store.data.projects).find(p => p.homeToken && p.homeToken === homeToken) || null;
}

// scenes of a project in sceneOrder (unknown ones appended) — shared ordering
function orderedScenes(project) {
    const all = (project && project.sources) || {};
    const order = Array.isArray(project && project.sceneOrder) ? project.sceneOrder : [];
    const seen = new Set();
    const out = [];
...
// operator's scripted auto-answers for a scene: one reply per line, blanks
// dropped. Sent back to the drop page as an incoming bot bubble after each of
// the visitor's contributions (line 1 after the 1st message, line 2 after the
// 2nd, …). Reads whatever is stored (admin caps line count/length on write).
function autoReplyLines(source) {
    return String((source && source.autoReplies) || '')
...
module.exports = {
    findSourceByDropToken, findProjectByHomeToken, orderedScenes,
    findMachineByToken, findSource, findStation,
    sourceDir, mediaUrl, mediaVersion, machinesForScene, autoReplyLines,
