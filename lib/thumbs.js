...
async function getThumb(absPath) {
    ...
    if (type === 'image') {
        ...
        await sharp(audioCardSvg(THUMB_W)).jpeg({ quality: 80 }).toFile(out);
    } else {
...
// A "voice note" tile for dropped audio: a waveform on a tinted card.
function audioCardSvg(size) {
    const cx = size / 2, cy = size / 2;
    const bars = [0.3, 0.55, 0.85, 0.45, 1, 0.7, 0.35, 0.6, 0.9, 0.5, 0.25, 0.65, 0.4];
    const bw = 10, gap = 9, maxH = size * 0.42;
    const totalW = bars.length * bw + (bars.length - 1) * gap;
    const startX = cx - totalW / 2;
    const rects = bars.map((v, i) => {
        const h = Math.max(8, v * maxH), x = startX + i * (bw + gap);
        return `<rect x="${Math.round(x)}" y="${Math.round(cy - h / 2)}" width="${bw}" height="${Math.round(h)}" rx="${bw / 2}" fill="#7cc4ff"/>`;
    }).join('');
    const svg =
        `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}">` +
        `<rect width="${size}" height="${size}" fill="#1b1e27"/>` +
        `<rect x="0" y="0" width="${size}" height="6" fill="#5b9dff"/>` +
        rects +
        `<text x="${size - 12}" y="${size - 14}" font-family="sans-serif" font-size="16" fill="#8a93a3" text-anchor="end">AUDIO</text>` +
        `</svg>`;
