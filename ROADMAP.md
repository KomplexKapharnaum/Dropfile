...
- **Extras** ✅ text + audio drop shown on players, admin playback remote, `/diag` WebRTC tester; per-scene **Intro** + scripted **Auto-answers** (posted back after each contribution, pushed live to open drop pages via `drop-meta`); a project **drop home** (`/h/:token`) menu of labelled scenes.
